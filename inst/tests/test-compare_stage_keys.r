...
test_that('it correctly compares a nested list to a linear list', {
  ...
  expect_false(compare_stage_keys(list(FALSE, list(list(FALSE, TRUE), FALSE), FALSE), list(FALSE, TRUE, TRUE)))
...
test_that('it correctly compares a nested list to a nested list', {
  ...
  expect_false(compare_stage_keys(list(list(FALSE, TRUE)), list(list(TRUE))))
})
